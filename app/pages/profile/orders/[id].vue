<template>
  <div class="mx-auto max-w-h3d-max px-6 py-10 sm:px-10 sm:py-14">
    ...
    <template v-else-if="!order">
      <div class="border border-dashed border-h3d-border bg-h3d-surface px-8 py-16 text-center">
        <h1 class="mb-3 font-h3d-display text-xl font-light text-h3d-text">Order not found</h1>
        <p class="font-h3d-body text-sm text-h3d-muted">ID <span class="text-h3d-text">{{ displayId }}</span> does not exist or you do not have permission to access it.</p>
      </div>
    </template>
          ...
          <section class="border border-h3d-border bg-h3d-surface" aria-labelledby="ship-heading">
            ...
            <div class="px-6 py-5">
              ...
              <p class="mt-3 font-h3d-body text-sm text-h3d-muted">
                Estimated: 2–3 weeks from order date
              </p>
            </div>
          </section>
...
const { data: rawOrder, pending } = await useFetch(() => `/api/profile/orders/${displayId.value}`)
const order = computed(() => rawOrder.value as any)

const orderDate = computed(() => {
  if (!order.value?.created_at) return '—'
  return new Date(order.value.created_at).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })
})
...
const lines = computed(() => {
  ...
  return list.map((l: any) => {
    ...
    const price = l.unit_price ?? l.unitPrice ?? 0
    return {
      name: l.name,
      meta: l.variant ?? l.meta ?? '',
      qty,
      unitPrice: price,
      lineTotal: l.lineTotal ?? (qty * price),
    }
  })
})
